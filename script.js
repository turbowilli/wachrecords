...
function setPicture(picture,name,alt=''){
  ...
  const pair=imagePair(name),source=picture.querySelector('source'),img=picture.querySelector('img');
...
function loadHome(c){
  ...
  if(magSection){
    const kicker=magSection.querySelector('.section-kicker');
    const title=magSection.querySelector('h2');
    const text=magSection.querySelector('.reveal p');
    const meta=magSection.querySelector('.meta');
    const link=magSection.querySelector('.text-link');
    if(kicker)kicker.textContent=mag.kicker;
    if(title)title.innerHTML=mag.title.replace('\\n','<br>');
    if(text)text.textContent=mag.text;
...
function loadPage(page,c){
  ...
  if(page==='contact'){const block=document.querySelector('.contact-block');if(block)block.innerHTML=`<a href="mailto:${c.email}">${c.email}</a>`;const note=document.querySelector('.text-page p');if(note)note.textContent=c.note;const purchase=document.querySelector('.purchase-note');if(purchase)purchase.textContent=c.purchase||'';}
  if(page==='impressum'){const ps=document.querySelectorAll('.text-page p'),note=document.querySelector('.legal-note');if(note)note.innerHTML=`<strong>Hinweis:</strong> ${c.legalNote}`;if(ps[0])ps[0].innerHTML=c.provider.replace(/\\n/g,'<br>');if(ps[1])ps[1].innerHTML=`${c.email}${c.phone?`<br>${c.phone}`:''}`;}
...
function initContent(){
  ...
  loadContentModules().then(()=>{
    ...
    if(path.endsWith('/magazine.html'))loadPage('magazine',magazineContent);
    if(path.endsWith('/exhibitions.html'))loadPage('exhibitions',exhibitionsContent);
    if(path.endsWith('/about.html'))loadPage('about',aboutContent);
